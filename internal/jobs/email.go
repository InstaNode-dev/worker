...
// SendDeployExpiring + SendDeployExpired removed 2026-05-14 — Wave FIX-I/J
// email migration. The audit_log rows (deploy.expiring_soon / deploy.expired)
// emitted by deployment_reminder.go and deployment_expirer.go are now the
// dispatch trigger; the BrevoForwarder (event_email_forwarder.go) picks
// them up on its next 60s tick and POSTs to Brevo using
// BREVO_TEMPLATE_IDS[kind]. See:
//   - internal/jobs/event_email_mapping.go — buildDeployExpiringSoon / buildDeployExpired
//   - internal/email/brevo_provider.go     — POST /v3/smtp/email shape
